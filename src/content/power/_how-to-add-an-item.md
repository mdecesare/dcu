# How to add a "Power & resilience" item
...
- `citation` and `sourceType` are optional. `sourceType` is R, P, V or G.
...
2. `showPowerAndResilience: true` is set in `src/site-settings.ts`.
